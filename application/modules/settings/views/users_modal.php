...
<div class="modal-body">
	...
	<form name="form" id="form" role="form" method="post" >
		...
		<div class="row">
			...
			<div class="col-sm-4">
				<div class="form-group text-left">
					...
					<input type="text" id="user" name="user" class="form-control" value="<?php echo $information?$information[0]["log_user"]:""; ?>" placeholder="Nombre Usuario" <?php if (!empty($information[0]["log_user"])) { ?> readOnly <?php } else { ?> required <?php } ?> />
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-4">
				<div class="form-group text-left">
					<label class="control-label" for="idSede">Punto Vive Digital: *</label>
					<select name="idSede" id="idSede" class="form-control" required>
						<option value="">Seleccione...</option>
						<?php for ($i = 0; $i < count($sedes); $i++) { ?>
							<option value="<?php echo $sedes[$i]["id_sede"]; ?>" <?php if($information && $information[0]["fk_id_sede"] == $sedes[$i]["id_sede"]) { echo "selected"; }  ?>><?php echo $sedes[$i]["nombre_sede"]; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="form-group text-left">
					...
					<input type="text" class="form-control" id="email" name="email" value="<?php echo $information?$information[0]["email"]:""; ?>" placeholder="Correo Electrónico" <?php if (!empty($information[0]["email"])) { ?> readOnly <?php } else { ?> required <?php } ?> />
				</div>
			</div>
			<div class="col-sm-4">
				<div class="form-group text-left">
		...
		<div class="row">
			<div class="col-sm-4">
				<div class="form-group text-left">
					<label class="control-label" for="idRole">Rol Usuario: *</label>
